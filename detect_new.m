function [MHW,mclim,m90,mhw_ts]=detect_new(temp,time,cli_start,cli_end,mhw_start,mhw_end,varargin)
%detect - Detecting spatial MHW/MCS
%  Syntax
%  Writer: Ce Bian
%  2023/12/24
%  Original writer: Zijie Zhao of function 'detect'
%
                ...
                if sum(isnan(squeeze(mbigadd(i,j,:))))~=size(mbigadd,3)
                    ...
                    if ~isempty(potential_event)
                        ...
                        if ~isempty(potential_event)
                            ...
                            mhwloc_max=NaN(size(potential_event,1),1);
                            for le=1:size(potential_event,1)
                                ...
                                [maxanom,maxloc]=nanmax(squeeze(manom));
                                mhwloc_max(le)=...
                                    maxloc;
                                mhwint_max(le)=...
                                    maxanom;
                            ...
                            MHW=[MHW;[mhwstart mhwend mduration mhwint_max mhwint_mean mhwint_var mhwint_cum repmat(i,size(mhwstart,1),1) repmat(j,size(mhwstart,1),1) mhwloc_max]];
                        end
                    end
                end
                    ...
                    if ~isempty(potential_event)
                        ...
                        if ~isempty(potential_event)
                            ...
                            for le=1:size(potential_event,1)
                                ...
                                [maxanom,maxloc]=nanmin(squeeze(manom));
                                mhwloc_max(le)=...
                                    maxloc;
                                mhwint_max(le)=...
                                    maxanom;
...
MHW=table(MHW(:,1),MHW(:,2),MHW(:,3),MHW(:,4),MHW(:,5),MHW(:,6),MHW(:,7),MHW(:,8),MHW(:,9), MHW(:,10),...
    'variablenames',{'mhw_onset','mhw_end','mhw_dur','int_max','int_mean','int_var','int_cum','xloc','yloc','maxloc'});
